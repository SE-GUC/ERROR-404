...
class App extends Component {
  ...
  delClub = id => {
    axios.delete(`/api/Clubs/${id}`)
      .then(this.setState({allClubs: [...this.state.allClubs.filter(club => club._id !== id)]}))
  ...
  updateClub = club => {
    ...
    for(i = 0; i < clubs.length; i++){
      if(clubs[i]._id === club._id){
        ...
        if(club.description !== "")
        clubs[i].description = club.description
      }
      
    }
    const updatedData = {}
    if(club.name !== "")
      updatedData.name = club.name
      if(club.description !== "")
      updatedData.description = club.description
    axios.put(`/api/Clubs/${club._id}`, updatedData)
  ...
  render() {
    return (
      <div className="App">
        ...
        <h1>UPDATE CLUB</h1>
        <UpdatingClubs allClubs = {this.state.allClubs} updateClub = {this.updateClub}/>
      </div>
    );
  }
}
