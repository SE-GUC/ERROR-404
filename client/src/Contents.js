...
class Contents extends Component {
  ...
  delContent = id => {
    axios.delete(`/api/Contents/${id}`)
      .then(this.setState({allContent: [...this.state.allContent.filter(content => content._id !== id)]}))    
  }
  updateContent = content => {
    ...
    const updatedData = {}
    if(content.name !== "")
      updatedData.name = content.name
      if(content.description !== "")
      updatedData.description = content.description
    axios.put(`/api/Contents/${content._id}`, updatedData)
  ...
  render() {
    return (
      <div className="App">
        ...
        <DeleteContent allContent = {this.state.allContent} delContent = {this.delContent}/>
        <UpdatingContents allContent = {this.state.allContent} updateContent = {this.updateContent}/>
      </div>
    );
  }
}
